...
module.exports.addTokwdIncome = function (req, res) {
  ...
  poolvalidate(req, function (retVal) {
    ...
    poolSchema.findOneAndUpdate({}, query).lean().exec(function (err, retVals) {
...
module.exports.addTotalKeywordOwnerPayout = function (req, res) {
  ...
  poolvalidate(req, function (retVal) {
    ...
    // Update Pool Fees Income Function
    poolSchema.findOneAndUpdate({}, {$inc: {'total_kwd_owner_payout': retVal[0].amount}}).lean().exec(function (err, retVals) {
      // Successfully Updated
      if (err) {
        log.error(err)
        return (err)
...
module.exports.getPoolStats = function (req, res) {
  ...
  poolSchema.find({}).lean().exec(function (err, retVal) {
    // Successfully Updated
    if (retVal[0]) {
